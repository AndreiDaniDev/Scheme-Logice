...
async def run5_Mondiala():
    ...
    await gyroFata(47, 600, kp = 0.4, kd = 0.25, resetGyro = False)
    await runloop.sleep_ms(250)
    await turnLeft(41.5, 650, 125, kp = 0.75, kd = 1)
    await runloop.sleep_ms(250)

    # ---> Comanda de 2 lei <---
    await gyroFata(5.5, 500, kp = 0.3, kd = 0.25, stop = False)
    await gyroFata(16.5, 600, kp = 0.3, kd = 0.25, resetGyro = False, stop = False)
    await gyroFata(3, 250, kp = 0.3, kd = 0.25, resetGyro = False)

    # ---> Vorbim de margina de eroare <---
    await runloop.sleep_ms(500)
    await turnRight(79.5, 500, 175, kp = 0.75, kd = 1.2, resetGyro = False)

    # ---> Dummy proof <---
    #    ->>> 20 lei <<<-
    await motor.run_for_degrees(hub.port.C, 300, 1100)
    await runloop.sleep_ms(250)
    await gyroFata(13.5, 800, target = 35, kp = 0.4, kd = 0.25)
...
async def run6_Mondiala():
    ...
    await turnRight(25, 500, 100, kp = 0.2, kd = 0)
    await gyroFata(120, 1000, -300, kp = 0.2, kd = 0.3)
    #sfarsit modificat
    return
